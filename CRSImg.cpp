...
void DisplayImage(CRSImage * pRSImg)
{
	...
	do
	{
		cout << "输入作为rgb通道的波段序号（当前共有" << nBands << "个波段）：" << endl;
	...
	do
	{
		cout << "是否进行亮度曲线拉伸？(0-不拉伸，1-线性拉伸，2-高对比（2%）拉伸)" << endl;
		cin >> Coefficient;
	...
	if (Coefficient == 2)
	{
		...
		for (int k = 0; k < 3; k++)
		{
			Npts = new long[256](); //初始化动态数组
			Total = new long[256]();
			Acc_Pct = new float[256]();
			for (int i = 0; i < nLines; i++)
				for (int j = 0; j < nSamples; j++)
					Npts[static_cast<int>(pppData[i][j][rgb[k]])] += 1; //统计每一点的亮度信息
			flagB = flagT = 1;
			for (int i = 0; i < 256; i++)
			{
				if (i)
					Total[i] = (Npts[i] + Total[i - 1]); //计算累计值
				else
					Total[i] = Npts[i];
...
bool CRSImage::ReadMetaData(const char* lpstrMetaFilePath)  //读取文件元数据
{
	...
	ifs.open(lpstrMetaFilePath, ios_base::in); //以只读方式打开元数据文件
	if (!ifs.is_open())
		return false;
	...
	while (!ifs.eof())   //end of file
	{
		...
		else if (strSubTxt == "interleave") //判断栅格数据类型
		{
			ss >> strSubTxt;
			ClearHeadTailSpace(strSubTxt);
			if (strSubTxt == "bsq")
...
bool CRSImage::ReadImgData(const char * lpstrImgFilePath) //读取图像数据
{
	ifstream ifs;
...
bool CRSImage::Open(const char* lpstrPath)
{
	...
	int	pos = strMetaFilePath.rfind('.'); //定位后缀名开始位
	if (pos >= 0)
	{
		strMetaFilePath = strMetaFilePath.substr(0, pos); //从零到后缀名切片
	}
	strMetaFilePath.append(".hdr"); //增加hdr后缀
	if (!ReadMetaData(strMetaFilePath.c_str())) //调用并读取元数据
	{
...
void CRSImage::ShowHistogram(int band) const
{
	...
	for (int i = 0; i < m_nLines; i++)
		for (int j = 0; j < m_nSamples; j++)
			if(static_cast<int>(m_pppData[i][j][k])<=100&& static_cast<int>(m_pppData[i][j][k])>0)
				Npts[static_cast<int>(m_pppData[i][j][k])] += 1; //统计每一点的亮度信息
	...
	for (int i = 0; i < 101; i++)
	{
		...
		Acc_Pct[i] = static_cast<float>(Total[i]) * 100 / Pixels;
		if (i == 0)
			cout << resetiosflags(ios::right) << setiosflags(ios::left) //设置左对齐
			<< setw(11) << setfill(' ') << m_sBandNames[k] << '\t';
		else
			cout << setw(11) << setfill(' ') << '\t';
		cout << resetiosflags(ios::left) << setiosflags(ios::right) //设置右对齐
			<< setw(5) << setfill(' ') << i << '\t'
			<< setw(7) << Npts[i]
	...
	//EasyX输出
	initgraph(540, 340);//创建图形界面
	line(20, 320, 20, 20);//纵坐标
	line(20, 320, 520, 320);//横坐标
	for (int i = 0; i < 100; i++)
		solidrectangle(20 + 5 * i, 320 - int(ceil(300 * Percent[i] / 10)), 20 + 5 * i + 5, 320);//通过画矩形来绘制出直方图
...
void Image_Operation::NewMatrix_Zoom(int startH, int endH, int startW, int endW)
{
	...
	{
		double srcRow = float(row)*rowRatio; //源目标点行数
		double i = floor(srcRow);
		double t = srcRow - i;
		for (int col = startW; col < endW; col++)
		{
			double srcCol = float(col)*colRatio; //源目标点列数
	...
	for (int row = startH; row < endH; row++)
	{
		for (int col = startW; col < endW; col++)
		{
			...
			for (int band = 0; band < m_nBands; band++)
				if (srcRow >= 0. && srcRow < m_nLines - 0.5 && srcCol >= 0. && srcCol < m_nSamples - 0.5) //保证原像素点在图像范围内
...
void Image_Operation::Rotate(int degree)
{
	...
	co = cos(angle);
	si = sin(angle);
	int rotateW = static_cast<int>(m_nSamples * fabs(co) + m_nLines * fabs(si)); //计算旋转后的高和宽
	int rotateH = static_cast<int>(m_nSamples * fabs(si) + m_nLines * fabs(co));
	colOff = rotateW / 2; //坐标偏移量
	rowOff = rotateH / 2;
	//生成新图像矩阵
